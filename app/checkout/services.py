...
def perform_checkout(session: Session, current_user: User):
    try:
        cart_items = session.query(Cart).filter(Cart.user_id == current_user.id).all() #filter cart items where user_id matches current user who is logged in.

        if not cart_items:
            logger.warning(f"Checkout attempt with empty cart by user_id={current_user.id}")
            raise HTTPException(status_code=400, detail="Cart is empty")

        total_amount = 0  # intially 0 will add  all itmes amnt
        order_items = []  #empty list . it will conntain order items
        ...
        for item in cart_items:
            ...
            item_total = item.product.price * item.quantity  # calculate total for each item
            total_amount += item_total 
            order_items.append(OrderItem(  # simple append to order_items list -> OrderItem model update 
        ...
        order = Order(  #full order obj creation
            user_id=current_user.id,
            total_amount=total_amount,
            status="paid",  # mock payment success
            created_at=datetime.utcnow(),
            items=order_items  #orderitems list use here
        )

        session.add(order)

        for item in cart_items:  # after checkout delete all items from cart
            session.delete(item)
