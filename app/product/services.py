...
from app.product.schemas import ProductCreate, ProductFilterParams
...
def get_products_by_current_admin(db: Session, current_user: User):
    try:
        logger.debug(f"Fetching products created by: {current_user.email}")
        products = db.query(Product).filter(Product.created_by == current_user.id).all()
        return products

    except Exception as e:
        logger.error(f"Error fetching admin's products: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to fetch admin's products")
...
def list_products_service(
    ...
    try:
        ...
        products = query.offset(filters.offset()).limit(filters.page_size).all()
        return products
    except Exception as e:
