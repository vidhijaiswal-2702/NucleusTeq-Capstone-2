...
@product_router.get("/admin/products", response_model=List[schemas.Product])
def list_admin_products(
    db: Session = Depends(get_session),
    current_user: User = Depends(admin_required)
):
    return services.get_products_by_current_admin(db, current_user)



@product_router.get("/all", response_model=list[schemas.Product])
def get_products(
    skip: int = 0,
    limit: int = 10,
    db: Session = Depends(get_session),
...
def list_products(
    filters: schemas.ProductFilterParams = Depends(),
    ...
    """
    return  services.list_products_service(session=session,filters=filters)
