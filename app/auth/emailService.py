...
async def send_account_verification_email(user: User, background_tasks: BackgroundTasks):
    ...
    activate_url = f"{settings.FRONTEND_HOST}/auth/account-verify?token={token}&email={user.email}" #ceate url for acc verify
    data = { #email content
...
async def send_account_activation_confirmation_email(user: User, background_tasks: BackgroundTasks): #sends welcome mail
    data = {
        'app_name': settings.APP_NAME,
        "name": user.name,
